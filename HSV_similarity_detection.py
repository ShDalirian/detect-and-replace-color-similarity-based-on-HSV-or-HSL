...
def background_color(img,background_ratio:int=50,dist:int=50,black_color=np.uint8([[45,47,60]]),white_color=np.uint8([[215,215,225]])):
    ...
    b_converted_img,Black_pixels=color_similarity_HSV(img,ideal_color1,[0,0,0],distance=dist)
    w_converted_img,white_pixels=color_similarity_HSV(img,ideal_color2,[0,0,255],distance=dist)
...
def color_similarity_HSV(img,specific_colors,ideal_color=[0,0,255],distance:int=15,sensivity:int=0,limit_other_colors:bool=False):
    masked_img=np.ones(img.shape, dtype=np.uint8)
...
def avr_HSV(RGB_sample):
    ...
    HSV_convert = np.zeros((RGB_sample_array1.shape[0],3))
